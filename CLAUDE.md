...
- **存储**: Cloudflare R2 (`lib/r2-storage.ts`,可选,用于文件上传)

### 开发工具
- **类型安全**: TypeScript (strict mode)
- **测试**: Vitest 4 + Testing Library (`tests/` 目录,配置见 `vitest.config.ts`)
- **代码规范**: ESLint
- **包管理**: pnpm
...
# 启动开发服务器
pnpm dev
pnpm dev:turbopack  # Turbopack 模式
pnpm dev:webpack    # Webpack 模式 (Turbopack 遇到问题时的兜底方案)
...
pnpm sync:fumadocs-style      # 同步 Fumadocs 样式 (生成 public/fumadocs-style.css,勿手写编辑)
```
...
RESEND_AUDIENCE_ID="可选,用于 Newsletter 受众管理"
...
- `STORAGE_*`: Cloudflare R2 存储配置 (STORAGE_REGION, STORAGE_BUCKET_NAME, STORAGE_ACCESS_KEY_ID, STORAGE_SECRET_ACCESS_KEY, STORAGE_ENDPOINT, STORAGE_PUBLIC_URL)
- `NEXT_PUBLIC_ENABLE_CHAT`: 聊天功能开关 (默认 `false`)
- `NEXT_PUBLIC_MAINTENANCE_MODE`: 维护模式开关 (默认 `false`)
